...
if __name__ == "__main__":
    ...
    while chunk:
        ...
        # (buffer size, current occupation, timestamp)
        buf_occ = buffer.get_student_params()
        # {Available_Bitrates: chunk_bitrates} of current chunk
        av_bitrates = prep_bitrates(manifest["Available_Bitrates"], chunk)
        # (number of remaining chunks, chunk time interval, current chunk number)
        ...
        chosen_bitrate = simulator_comm.send_req_json(
            m_band, prev_throughput, buf_occ, av_bitrates, current_time, chunk_arg, rebuff_time, pref_bitrate)
        ...
        logger.log_bitrate_choice(
            current_time, chunknum, (chosen_bitrate, stu_chunk_size))

        # simulate download and playback
        time_elapsed = trace.simulate_download_from_time(
            current_time, stu_chunk_size)

        # round time to remove floating point errors
        # todo: this did not fix them
        time_elapsed = round(time_elapsed, 3)

        rebuff_time = buffer.sim_chunk_download(
            stu_chunk_size, chunk_arg["time"], time_elapsed)
