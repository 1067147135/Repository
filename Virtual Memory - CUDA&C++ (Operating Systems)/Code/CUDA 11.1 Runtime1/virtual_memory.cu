...
__device__ void init_invert_page_table(VirtualMemory *vm) {
  ...
  for (int i = 0; i < vm->PAGE_ENTRIES; i++) {  // PAGE_ENTRIES = 1024
    vm->invert_page_table[i] = 1;   // valid bit: invalid: = 1, valid = 0
    vm->invert_page_table[i + vm->PAGE_ENTRIES] = i;    // virtual address
...
__device__ uchar vm_read(VirtualMemory *vm, u32 addr) {
    ...
    for (int i = 0; i < 1024; i++) {    // if the data need to be read is in the buffer
        if (vm->invert_page_table[i] == 0 && vm->invert_page_table[i + vm->PAGE_ENTRIES] == addr) {
    ...
    u32 vpn = vm->invert_page_table[address + vm->PAGE_ENTRIES];
    for (int i = 0; i < 32; i++) {
    ...
    vm->invert_page_table[address + vm->PAGE_ENTRIES] = addr;
...
__device__ void vm_write(VirtualMemory *vm, u32 addr, uchar value) {
    ...
    if (vm->pointer < 1024) {   // pointer point to the next physical address to be filled before physical memory filled
        ...
        vm->invert_page_table[vm->pointer] = 0; // set the valid bit
        vm->invert_page_table[vm->pointer + vm->PAGE_ENTRIES] = addr;   // store the virtual address
