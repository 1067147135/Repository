...
# find the best way to divide, return the boundary and the gini number
...
# code to make tree
def make_tree(dataset, yes, no, gini):
    global columns
    global node
    ...
    compare.sort()
    left_side = []  # [column_name, < boundary, data_set1, gini, yes1, no1, tag]
    ...
    if compare[0][0][2] >= compare[0][0][3]:
        ...
        left_side.append('No')
    right_side = [] # [column_name, > boundary, data_set2, gini, yes2, no2, tag]
    ...
    left = left_side[:]
    right = right_side[:]
    tree.add_left(node,left) # use a recursive loop to build a tree
    tree.add_right(node,right)
    node = node.left
    print('working...')
    make_tree(node.element[2], node.element[4], node.element[5], node.element[3])
    node = node.parent
    node = node.right
    print('working...')
    make_tree(node.element[2], node.element[4], node.element[5], node.element[3])
...
while True:
    ...
    if line:
        ...
        if data[-1][-1] == '\n':
            data[-1] = data[-1][:-1]
        data_list.append(dict(zip(title[:],data[:])))
    else:
        break
train_data.close()
print('finish!')
