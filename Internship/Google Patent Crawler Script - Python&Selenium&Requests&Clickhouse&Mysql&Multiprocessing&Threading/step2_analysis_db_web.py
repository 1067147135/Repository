...
input_folder_path = '/home/swl/bopu/data/CN_2022_p'   # path to folder
...
def operate(pid, tid, rows, chunks, file_name, logger, d: data):
   
    client = Client(host='10.25.1.22', port='9000', database='his_data_snaps', user='bdf', password='bdf2022')
    ...
    for i in range(chunks[tid][0], chunks[tid][1]):
        ...
        # get from database
        query_common = "select * from his_data_snaps.google_patent_data_common where result_link = '{}'".format(url)
        data_common = client.execute(query_common)
        if len(data_common) == 1:
            d.abstr_list[i] = data_common[0][10]
            d.claims_list[i] = data_common[0][11]
            d.clas_list[i] = data_common[0][12]

            query_cite = "select * from his_data_snaps.google_patent_data_cite where patent_id = '{}'".format(id)
            data_cite = client.execute(query_cite)
            n = len(data_cite)
            if n > 0:
                # print('line', i, 'has cite | ', len(d.id_list))

                for j in range(n):
                    d.id_list[tid].append(id)
                    d.pub_number_list[tid].append(data_cite[j][1])
                    d.pri_date_list[tid].append(data_cite[j][2])
                    d.pub_date_list[tid].append(data_cite[j][3])
                    d.assignee_list[tid].append(data_cite[j][4])
                    d.title_list[tid].append(data_cite[j][5])
