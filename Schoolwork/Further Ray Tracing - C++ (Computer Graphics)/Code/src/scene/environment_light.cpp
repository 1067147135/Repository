...
namespace CGL { namespace SceneObjects {
  ...
  void EnvironmentLight::init() {
    ...
    // normalize and update marginal_y 
    double s = 0;
    for (int j = 0; j < h; ++j) {
      for (int i = 0; i < w; ++i) {
    ...
    for (int i = 0; i < w; ++i){
      conds_y[i] = pdf_envmap[i] / marginal_y[0];
    }
    for (int j = 0; j < h; ++j){
      conds_y[w * j] = pdf_envmap[w * j] / marginal_y[j];
    }
    for (int j = 1; j < h; ++j) {
      double denominator = marginal_y[j] - marginal_y[j - 1];
      for (int i = 1; i < w; ++i) {
        conds_y[w * j + i] = pdf_envmap[w * j + i] / denominator + conds_y[w * j + i - 1];
      }
    }
  ...
  Vector3D EnvironmentLight::sample_L(const Vector3D p, Vector3D* wi,
    ...
    // Importance sampling
    Vector2D ab = sampler_uniform2d.get_sample();
    Vector2D xy;
    int i_x, i_y;
    uint32_t w = envMap->w, h = envMap->h;
    // std::cout << "w: " << w << ", h: " << h << std::endl;

    double *y = std::upper_bound(marginal_y, marginal_y + h, ab.y);
    i_y = y - marginal_y;
    // xy.y = (double)i_y / h;
    xy.y = i_y;
    if (i_y >= h) i_y = h - 1;
    double *x = std::upper_bound(conds_y + w * i_y, conds_y + w * (i_y + 1), ab.x);
    i_x = x - (conds_y + w * i_y);
    // xy.x = (double)i_x / w;
    xy.x = i_x;
    if (i_x >= w) i_x = w - 1;
    ...
    *distToLight = INF_D;
    *pdf = pdf_envmap[i_x + i_y * w] * w * h / (2 * PI * PI * sin(theta_phi.x));
    Vector3D radiance = bilerp(xy);
