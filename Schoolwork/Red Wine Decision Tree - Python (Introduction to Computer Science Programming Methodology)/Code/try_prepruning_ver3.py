...
# find the best way to divide, return gini number, boundary, two subsets of train set and subsets' yes/no
# and two subsets of validation set.
def find_criterion(dataset,column,validation):
    sorting = []
    for item in dataset:
    ...
    new_dataset3 = []   # two subsets of validation set
    new_dataset4 = []
    for item in validation:
        if float(item[column]) < boundary_final:
            new_dataset3.append(item)
        else:
            new_dataset4.append(item)
    return gini_final, boundary_final, type1_yes_final, type1_no_final, type2_yes_final, type2_no_final, new_dataset1, new_dataset2, new_dataset3, new_dataset4
...
# code to make tree
def make_tree(dataset, yes, no, gini, accuracy, validation):
    global columns
    global node
    ...
    for column in columns: 
        compare.append((find_criterion(dataset,column,validation),column))
    compare.sort()    
    left_side = []  # [column_name, < boundary, data_set1, gini, yes1, no1, tag, data_set3, accuracy1]
    ...
    if compare[0][0][2] >= compare[0][0][3]:
        ...
        left_side.append('No')
    left_side.append(compare[0][0][8])
    right_side = [] # [column_name, > boundary, data_set2, gini, yes2, no2, tag, data_set4, accuracy2]
    ...
    right_side.append(compare[0][0][9])
    correct1 = 0    # measure accuracy based on verification set
    wrong1 = 0
    correct2 = 0
    wrong2 = 0
    if left_side[6] == 'Yes':
        for item in left_side[7]:
            if float(item['quality']) > 6:
                correct1 += 1
            else:
                wrong1 +=1
        accuracy1 = correct1 / (correct1 + wrong1)
    else:
        for item in left_side[7]:
            if float(item['quality']) > 6:
                wrong1 += 1
        ...
        accuracy1 = correct1 / (correct1 + wrong1)
    if right_side[6] == 'Yes':
        for item in right_side[7]:
            if float(item['quality']) > 6:
                correct2 += 1
            else:
                wrong2 += 1
        accuracy2 = correct2 / (correct2 + wrong2)
    else:
        for item in right_side[7]:
            if float(item['quality']) > 6:
                wrong2 += 1
                ...
                correct2 +=1
        accuracy2 = correct2 / (correct2 + wrong2)
    accuracy_now = (correct1 + correct2) / (correct1 + correct2 + wrong1 + wrong2)
    ...
    node = node.left
    print('working...')
    make_tree(node.element[2], node.element[4], node.element[5], node.element[3], node.element[8], node.element[7]) # use a recursive loop to build a tree
    node = node.parent
    node = node.right
    print('working...')
    make_tree(node.element[2], node.element[4], node.element[5], node.element[3], node.element[8], node.element[7])
    node = node.parent
    return node
...
while True:
    ...
    if line:
        ...
        if count <= 500:
            ...
            validation_set.append(dict(zip(title[:],data[:])))  # creat a validation set
    else:
        break
train_data.close()
print('finish!')
...
make_tree(data_list, 0, 0, 1, 0, validation_set)
print('finish!')
