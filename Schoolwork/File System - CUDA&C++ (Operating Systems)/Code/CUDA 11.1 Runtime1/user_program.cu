...
__device__ void user_program(FileSystem *fs, uchar *input, uchar *output) {
	...
	*/
	...
	for (int k = 2; k < 15; k++)
		for (int i = 50; i <= 126; i++, p++)	// 13 * 77 = 1001
		{
			fname2[p][0] = i;
			for (int j = 1; j < k; j++)
				fname2[p][j] = 64 + j;
	...
	/*
	////////////// Test case 4 ////////////////
	char fname1[540][20];	//31block文件
	for (int i = 1; i < 550; i++)
	{
		//存31block的文件,文件名为9001-9500
		int temp = i;
		int count = 0;
		fname1[i - 1][0] = '0';
		fname1[i - 1][1] = '0';
		...
		fname1[i - 1][0] = '9';
		if (i <= 500) {
			u32 fp = fs_open(fs, fname1[i - 1], G_WRITE);
			fs_write(fs, input + i, 992, fp);
		}
		else if (i == 501) {
			for (int j = 1; j < 501; j++) {
				//删除31block的文件
				fname1[j - 1][0] = '9';
				fs_gsys(fs, RM, fname1[j - 1]);
			}
		}
