...
Ray Camera::generate_ray(double x, double y) const {
  ...
  Vector4D SensorPos = {(x - 0.5) * tan(0.5 * radians(hFov)) * 2, (y - 0.5) * tan(0.5 * radians(vFov)) * 2, -1, 1};
  double data[16] = {c2w(0,0), c2w(0,1), c2w(0,2), pos[0], c2w(1,0), c2w(1,1), c2w(1,2), pos[1],  c2w(2,0), c2w(2,1), c2w(2,2), pos[2], 0, 0, 0, 1};
  Matrix4x4 cameraToWorld(data);
  Vector4D WorldPos = cameraToWorld * SensorPos;
  Vector3D WorldDir = {WorldPos[0] - pos[0], WorldPos[1] - pos[1], WorldPos[2] - pos[2]};

  // Vector3D SensorPos = {(x - 0.5) * tan(0.5 * radians(hFov)) * 2, (y - 0.5) * tan(0.5 * radians(vFov)) * 2, -1};
  // Vector3D WorldDir = c2w * SensorPos;
  Ray r({WorldPos[0], WorldPos[1], WorldPos[2]}, WorldDir.unit());
  r.min_t = nClip;
  r.max_t = fClip;
  return r;
}
