...
bool shouldCal(int i, bool* fire_area) {
...
void update(float *data, float *new_data, bool *fire_area, int start, int end) {
    ...
    for (int i = 1; i < (end - start - 1); i++){
        for (int j = 0; j < size; j++){
            int idx = (i + start) * size + j;
            if (shouldCal(idx, fire_area)) {
                idx = i * size + j;
...
void slave(){
    // TODO: MPI routine
    float* local_old = new float[102 * size];
    float* local_new = new float[102 * size];
    bool* local_fire = new bool[size * size];

    generate_fire_area(local_fire);
    ...
    while (true) {
        ...
        MPI_Recv(local_old, n_items, MPI_FLOAT, 0, status.MPI_TAG+1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        ...
        MPI_Send(work, 2, MPI_INT, 0, status.MPI_TAG+2, MPI_COMM_WORLD);
        // printf("Slave %d: notify finish workitem from %d to %d\n", my_rank, work[0], work[1]);
        // send the result
        MPI_Send(&local_new[size], n_items- size * 2, MPI_FLOAT, 0, status.MPI_TAG+3, MPI_COMM_WORLD); 
...
void master() {
    ...
    MPI_Status status;
    MPI_Request request_send[(world_size - 1) * 2];
    ...
    // while (cont) {
        ...
        for (int rank = 1; rank < world_size; ++rank) {
            ...
            int start = workitems[rank-1].first - 1;
            int end = workitems[rank-1].second + 1;
            int n_items = (end - start) * size;
            int work[2] = {start, end};
        
            // Send it to each rank 
            // tag = 0: exit; ; tag = other: (workitem index+1) * 4 + (0~3)
            // +0: send workload; +1: send data; +2: notify result ready; +3: send result
            MPI_Isend(work, 2, MPI_INT, rank, rank*4, MPI_COMM_WORLD, &request_send[(rank - 1) * 2]); 
            // printf("Master: send workitem from %d to %d -> slave %d with tag %d\n", work[0], work[1], rank, rank*4);
            if (count % 2 == 1) MPI_Isend(&data_odd[start * size], n_items, MPI_FLOAT, rank, rank*4+1, MPI_COMM_WORLD, &request_send[(rank - 1) * 2 + 1]);
            else MPI_Isend(&data_even[start * size], n_items, MPI_FLOAT, rank, rank*4+1, MPI_COMM_WORLD, &request_send[(rank - 1) * 2 + 1]);
            // printf("Master: send data from %d to %d -> slave %d with tag %d\n", work[0], work[1], rank, rank*4+1);
        }
        ...
        while (index < workitems.size()){
            ...
            // printf("Master: get result from slave %d workitem from %d to %d is ready with tag %d\n",status.MPI_SOURCE, res[0], res[1], status.MPI_TAG);
            
            // Send the slave a new work unit 
            start = workitems[index].first - 1;
            end = workitems[index].second + 1;
            n_items = (end - start) * size;
            int work[2] = {start, end};
            MPI_Send(work, 2, MPI_INT, status.MPI_SOURCE, (index+1)*4, MPI_COMM_WORLD); 
            if (count % 2 == 1) MPI_Send(&data_odd[start * size], n_items, MPI_FLOAT, status.MPI_SOURCE, (index+1)*4+1, MPI_COMM_WORLD);
            else MPI_Send(&data_even[start * size], n_items, MPI_FLOAT, status.MPI_SOURCE, (index+1)*4+1, MPI_COMM_WORLD);
            // printf("Master: send workitem data from %d to %d -> slave %d with tag %d\n", work[0], work[1], status.MPI_SOURCE, (index+1)*4+1);
        ...
        // printf("check: data_even = %f, data_odd = %f\n", data_even[2000], data_odd[2000]);
